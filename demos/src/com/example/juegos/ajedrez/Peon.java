...
public class Peon extends Pieza {
	 ...
	 * @param onPromocion Delegado a ejecutar para solicitar al usuario la pieza para la promoción.
	 */
	...
	/**
	 * Reemplazado. Indica porque el Peón no puede realizar el movimiento.
	 * Verifica que el peón avanza. Si el movimiento es vertical comprueba que solo avance una posición (dos, en caso de apertura, sin saltar otras piezas) siempre que el escaque de destino no esté ocupado. En caso de ser diagonal, verifica que solo avance una posición y pueda capturar una pieza contraria. NO CONTEMPLA LA CAPTURA AL PASO.
	 * @param movimiento Movimiento a verificar.
	...
	public String getError(Movimiento movimiento, Tablero tablero) {
		...
		else if (movimiento.esDiagonal() && !puedeComer(movimiento, tablero))
	...
	public void mover(Movimiento movimiento, Tablero tablero) throws JuegoException {
		...
		} else throw new JuegoException(getError(movimiento, tablero));
	}
